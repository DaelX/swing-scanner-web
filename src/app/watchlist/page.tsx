...
function SignalBadge({ signal }: { signal: Signal }) {
  ...
  return <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${styles[signal]}`}>{labels[signal]}</span>;
}

function TrendBadge({ trend }: { trend: "BULL" | "BEAR" | "RANGE" }) {
  const styles = { BULL: "text-green-400", BEAR: "text-red-400", RANGE: "text-yellow-400" };
...
export default function WatchlistPage() {
  ...
  useEffect(() => {
    fetch("/api/watchlist")
      ...
      .then((data: DashboardResponse) => { setStocks(data.stocks); setScanDate(data.scan_date); })
  ...
  const SortHeader = ({ k, children }: { k: SortKey; children: React.ReactNode }) => (
    <th className="px-2 py-2 text-left text-[10px] uppercase tracking-wider text-slate-400 cursor-pointer hover:text-white select-none" onClick={() => toggleSort(k)}>
      <div className="flex items-center gap-1">{children}{sortKey === k && <span>{sortDir === "desc" ? "▼" : "▲"}</span>}</div>
    </th>
  );
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
        <div>
          ...
          <p className="text-sm text-slate-400">{scanDate ? `${scanDate} · ${stocks.length} stocks tracked` : "Loading..."}</p>
        </div>
      </div>
      ...
      {stocks.length > 0 && (
        <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-4">
          <div className="bg-green-900/30 border border-green-700 rounded-lg p-3 text-center cursor-pointer hover:border-green-500 transition" onClick={() => setFilter(filter === "STRONG_BUY" ? "all" : "STRONG_BUY")}>
            ...
            <div className="text-[10px] text-green-500 uppercase">Strong Buy</div>
          </div>
          <div className="bg-green-900/15 border border-green-800 rounded-lg p-3 text-center cursor-pointer hover:border-green-600 transition" onClick={() => setFilter(filter === "BUY" ? "all" : "BUY")}>
            ...
            <div className="text-[10px] text-green-600 uppercase">Buy</div>
          </div>
          ...
          <div className="bg-red-900/15 border border-red-800 rounded-lg p-3 text-center cursor-pointer hover:border-red-600 transition" onClick={() => setFilter(filter === "SELL" ? "all" : "SELL")}>
            ...
            <div className="text-[10px] text-red-600 uppercase">Sell</div>
          </div>
          <div className="bg-red-900/30 border border-red-700 rounded-lg p-3 text-center cursor-pointer hover:border-red-500 transition" onClick={() => setFilter(filter === "STRONG_SELL" ? "all" : "STRONG_SELL")}>
            ...
            <div className="text-[10px] text-red-500 uppercase">Strong Sell</div>
          </div>
        </div>
      )}
        ...
        <div className="overflow-x-auto border border-slate-700 rounded-lg">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((s) => (
                <>
                  <tr key={s.symbol} className={`border-b border-slate-800 hover:bg-slate-800/50 cursor-pointer transition ${s.signal === "STRONG_BUY" ? "bg-green-950/20" : s.signal === "STRONG_SELL" ? "bg-red-950/20" : ""}`} onClick={() => setExpandedRow(expandedRow === s.symbol ? null : s.symbol)}>
                    ...
                    <td className="px-2 py-2">{s.stop_loss ? <span className="font-mono text-xs text-red-400">${s.stop_loss.toFixed(2)}</span> : <span className="text-slate-600">—</span>}</td>
                    <td className="px-2 py-2">{s.target_1 ? <span className="font-mono text-xs text-green-400">${s.target_1.toFixed(2)}</span> : <span className="text-slate-600">—</span>}</td>
                    <td className="px-2 py-2">{s.reward_risk !== null ? <span className={`font-mono text-xs font-bold ${s.reward_risk >= 2 ? "text-green-400" : "text-yellow-400"}`}>{s.reward_risk.toFixed(1)}</span> : <span className="text-slate-600">—</span>}</td>
                    <td className="px-2 py-2">{s.position_size_pct !== null ? <span className="font-mono text-xs text-slate-300">{s.position_size_pct.toFixed(0)}%</span> : <span className="text-slate-600">—</span>}</td>
                  </tr>
      ...
      {!loading && stocks.length === 0 && !error && (
        <div className="bg-slate-800/30 border border-slate-700 rounded-lg p-8 text-center text-slate-400">
          No watchlist data yet. Run the cache writer to populate.
        </div>
      )}
